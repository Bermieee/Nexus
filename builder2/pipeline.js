...
import { builderDecisionFingerprint, evaluateBuilderParentPlacementAssist } from './decision-sites.js';
...
export class Builder2Pipeline {
  ...
  async #continueReusableStart(runId,candidate){
    ...
    if(!taxonomy){
      ...
      plan=await this.store.transition(runId,BUILDER2_PHASE.TAXONOMY_REVIEW,{taxonomy:nextTaxonomy,taxonomyRevision:nextTaxonomy.revisionId});return this.reviewRequest(plan);
    }
    if(!taxonomyApproved){
    ...
    const metadata={...(plan.metadata||{}),artifactReuse:{fromRunId:candidate.runId,survey:true,taxonomy:true,classificationCount:reused.classifications.length,missingClassificationCount:missing.length}};
    if(missing.length){
      plan=await this.store.transition(runId,BUILDER2_PHASE.CLASSIFICATION,{survey,taxonomy,taxonomyRevision:taxonomy.revisionId,taxonomyReview:{approved:true,reusedFromRunId:candidate.runId},classifications:reused.classifications,classificationPlanning:{pendingSourceKeys:missing,status:'pending',reason:'artifact-reuse-miss'},metadata});
      return this.#runClassification(runId,{onlySourceKeys:missing,merge:true});
    }
    const ambiguous=reused.classifications.filter(row=>row.decision===BUILDER2_CLASSIFICATION_DECISION.AMBIGUOUS);
    if(ambiguous.length){
      plan=await this.store.transition(runId,BUILDER2_PHASE.CLASSIFICATION_REVIEW,{survey,taxonomy,taxonomyRevision:taxonomy.revisionId,taxonomyReview:{approved:true,reusedFromRunId:candidate.runId},classifications:reused.classifications,classificationReview:{pending:ambiguous.map(row=>row.sourceKey),reusedFromRunId:candidate.runId},metadata});return this.reviewRequest(plan);
    }
    const gaps=collectBuilder2Gaps(reused.classifications);
    ...
    if(reusableGapReview){
      plan=await this.store.transition(runId,BUILDER2_PHASE.GAP_REVIEW,{survey,taxonomy,taxonomyRevision:taxonomy.revisionId,taxonomyReview:{approved:true,reusedFromRunId:candidate.runId},classifications:reused.classifications,taxonomyGaps:gaps,proposedExpansions:structuredClone(candidate.proposedExpansions),gapPlanning:{...(candidate.gapPlanning||{}),reusedFromRunId:candidate.runId},metadata});return this.reviewRequest(plan);
    }
    if(gaps.length){
      plan=await this.store.transition(runId,BUILDER2_PHASE.CLASSIFICATION,{survey,taxonomy,taxonomyRevision:taxonomy.revisionId,taxonomyReview:{approved:true,reusedFromRunId:candidate.runId},classifications:reused.classifications,metadata});return this.#prepareGaps(runId,reused.classifications);
    }
    if(candidate.phase===BUILDER2_PHASE.RECONCILIATION&&candidate.reconciliation?.proposals?.length){
      plan=await this.store.transition(runId,BUILDER2_PHASE.RECONCILIATION,{survey,taxonomy,taxonomyRevision:taxonomy.revisionId,taxonomyReview:{approved:true,reusedFromRunId:candidate.runId},classifications:reused.classifications,reconciliation:structuredClone(candidate.reconciliation),metadata});return this.reviewRequest(plan);
    }
    plan=await this.store.transition(runId,BUILDER2_PHASE.RECLASSIFICATION,{survey,taxonomy,taxonomyRevision:taxonomy.revisionId,taxonomyReview:{approved:true,reusedFromRunId:candidate.runId},classifications:reused.classifications,reconciliation:phaseAtLeast(candidate.phase,BUILDER2_PHASE.QUALITY_REVIEW)?structuredClone(candidate.reconciliation):null,metadata});
    if(phaseAtLeast(candidate.phase,BUILDER2_PHASE.QUALITY_REVIEW))return this.#runQuality(runId,{taxonomy,classifications:reused.classifications,reconciliation:structuredClone(candidate.reconciliation)});
    return this.#prepareReconciliation(runId,reused.classifications,{taxonomy});
  }
  ...
  async #runSurveyTaxonomy(runId){
    ...
    plan=await this.#readFresh(runId,'post-taxonomy-plan');plan=await this.store.transition(runId,BUILDER2_PHASE.TAXONOMY_REVIEW,{taxonomy,taxonomyRevision:taxonomy.revisionId});recordBuilder2Cycle(runId,'review',1,{kind:'taxonomy-review'});return this.reviewRequest(plan);
  }
  ...
  async #runClassification(runId,{onlySourceKeys=null,merge=false}={}){
    ...
    if(progress.stalled&&reusableRows.length===requested.length){
      ...
      const classifications=merge?plan.classifications:reusableRows;
    ...
    if((plan.taxonomy?.nodes||[]).length<=16&&legalTaxa.length){
    ...
    plan=await this.#readFresh(runId,'post-classification');const classifications=merge?replaceByKey(plan.classifications,rows):rows;const classificationProgress=builder2SemanticProgressMetadata(plan,{stage:'classification',fingerprint:roundFingerprint,reason:'authoritative-classification-round-complete'});
  ...
  async #prepareGaps(runId,classifications){
    let plan=await this.#readFresh(runId,'gap-detection');const ctx=await this.#context(plan);const gaps=collectBuilder2Gaps(classifications);if(!gaps.length)return this.#prepareReconciliation(runId,classifications);
    ...
    for(const proposed of proposals.filter(p=>!p.fallback).slice(0,24)){
      const evidenceSources=(proposed.evidenceSourceKeys||[]).map(key=>ctx.worksetSources.find(source=>source.sourceKey===key)).filter(Boolean).slice(0,8),candidates=(plan.taxonomy?.nodes||[]).filter(node=>node.taxonId!==proposed.taxonId).slice(0,16);if(!evidenceSources.length)continue;
    }
    recordBuilder2Cycle(runId,'review',1,{kind:'gap-review',nonProgressStalled});
    plan=await this.store.transition(runId,BUILDER2_PHASE.GAP_REVIEW,{classifications,taxonomyGaps:gaps,proposedExpansions:proposals,metadata:builder2SemanticProgressMetadata(plan,{stage:'gap-consolidation',fingerprint:roundFingerprint,stalled:nonProgressStalled,reason:nonProgressStalled?'identical gap round routed to deterministic operator fallback':'authoritative-gap-consolidation-complete'}),gapPlanning:{proposalCount:proposals.length,fallbackProposalCount,coverageComplete:proposals.reduce((n,p)=>n+(p.evidenceSourceKeys||[]).length,0)===gaps.length,nonProgressStalled,reason:nonProgressStalled?'identical semantic round; model consolidation skipped and operator fallback surfaced':null}});return this.reviewRequest(plan);
  }
  ...
  async #runQuality(runId,patch={}){let plan=await this.#readFresh(runId,'quality');const ctx=await this.#context(plan);const taxonomy=patch.taxonomy||plan.taxonomy,classifications=patch.classifications||plan.classifications;
    ...
    // final Quality Gate. This also migrates old persisted blocked-quality runs
    // back to the correct owner without replaying Survey/Taxonomy/Classification.
    ...
    const prospective=buildBuilder2ProspectivePopulation({treeInventory:ctx.treeInventory,worksetSources:ctx.worksetSources,classifications,taxonomy});const report=evaluateBuilder2Quality({sources:ctx.worksetSources,taxonomy,classifications,prospectivePopulation:prospective});plan=await this.store.transition(runId,BUILDER2_PHASE.QUALITY_REVIEW,{taxonomy,taxonomyRevision:taxonomy.revisionId,classifications,reconciliation:patch.reconciliation||plan.reconciliation,qualityReview:{report,status:report.passed?'review':'blocked'}});return this.reviewRequest(plan);}
...
export async function runBuilder2AutoToStage({pipeline,input,reviewers={}}={}){
  ...
  while(step?.reviewKind&&guard++<30){const p=step.plan,token=step.reviewToken;switch(step.reviewKind){case'taxonomy-review':step=await pipeline.reviewTaxonomy(p.runId,{token,...(await reviewers.taxonomy?.(p)||{})});break;case'classification-review':step=await pipeline.reviewClassifications(p.runId,{token,decisions:(await reviewers.classification?.(p))||{}});break;case'gap-review':step=await pipeline.reviewGaps(p.runId,{token,decisions:(await reviewers.gap?.(p))||{}});break;case'reconciliation-review':step=await pipeline.reviewReconciliation(p.runId,{token,decisions:(await reviewers.reconciliation?.(p))||{}});break;case'quality-review':step=await pipeline.reviewQuality(p.runId,{token,approved:(await reviewers.quality?.(p))?.approved!==false});break;case'preview':return pipeline.reviewPreview(p.runId,{token,approved:(await reviewers.preview?.(p))?.approved!==false});default:throw new Error(`Unknown review kind ${step.reviewKind}`);}}
