...
function recordCard(r){
    ...
    const digest=!digestDisabled?`<select class="text_pole tv2-memory-digest-destination"><option value="">Digest…</option><option value="lore">Digest → Lore Proposals</option><option value="notebook">Digest to Notebook</option></select>`:'';
...
function summaryRecordNode(r,{archive=false}={}){
    ...
    if(!archive){
        const actions=[];
        if(!queueActive&&r.permanent!==true&&r.locked!==true){const digest=nxSelect({value:'',options:[{value:'',label:'Digest…'},{value:'lore',label:'Digest → Lore Proposals'},{value:'notebook',label:'Digest → Notebook'}],className:'nx-summary-digest'});digest.controlElement.classList.add('tv2-memory-digest-destination');actions.push(digest);}
