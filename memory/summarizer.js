    ...
    for(let round=0;round<12;round++){
        ...
        for(const group of groups){
            ...
            const result=await runLaneAModelWorkerBatch({
                ...
                buildRecovery:(item)=>structuredSidecarOptions({prompt:`${prompt}\n\nRECOVERY: The prior response violated the structured evidence contract. Return one corrected JSON object only; preserve all allowed evidence IDs that support retained events. ${SUMMARY_METADATA_ARRAY_RULE}`,systemPrompt:'Return corrected structured evidence JSON only.',maxTokens:Math.min(2048,packing.resourcePolicy.outputCeilingTokens||2048),priority:BUS_PRIORITY.SUMMARY,structuredValidator:value=>validateSummaryEvidencePayload(value,item.allowed),telemetry:{summaryReshape:true,phase:'evidence-consolidation-recovery',round,group:group.index}}),
            });
    ...
    try{
        ...
        if(directPromptTokens<=SUMMARY_DIRECT_INPUT_TARGET){
            ...
            const batch=await runLaneAModelWorkerBatch({laneAEnqueue:enqueueSidecar,domain:NEXUS_BATCH_DOMAIN.MEMORY_BANK,stage:BUS_STAGE.SUMMARY,items:slices,requestedBatch:slices.length>1,label:`Summary evidence · messages ${plan.start}-${plan.end}`,priority:BUS_PRIORITY.SUMMARY,dedupKey:`summary-evidence:${getContext()?.chatId||''}:${plan.start}:${plan.end}`,telemetry:{memoryLayer:0,cycleId,manual,summaryReshape:true,...(directorMeta||{})},
                ...
                buildRecovery:slice=>{const ids=slice.rows.map(row=>row.evidenceId),prompt=`${summarySlicePrompt(slice.passage,ids,characterFocus)}\n\nRECOVERY: Correct the structured evidence contract only; do not broaden beyond this exact slice. ${SUMMARY_METADATA_ARRAY_RULE}`;return structuredSidecarOptions({prompt,systemPrompt:'Return corrected summary evidence JSON only.',maxTokens:Math.min(2048,packing.resourcePolicy.outputCeilingTokens||2048),priority:BUS_PRIORITY.SUMMARY,structuredValidator:value=>validateSummaryEvidencePayload(value,ids),telemetry:{summaryReshape:true,phase:'slice-recovery',sliceId:`summary-slice-${slice.index}`}});}});
