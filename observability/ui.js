...
function coordinationSnapshotHtml(telemetry = null) {
    ...
    const frameComparisonText=frame?.comparisonResetReason?`comparison reset: ${frame.comparisonResetReason}`:(frame?.hasPriorComparison===true?`first change ${frame.firstChangedSection||'none'}`:'no prior comparable frame');
    const frameDetail = frame ? `${frameComparisonText} · ${stranded?`${formatTokenCount(stranded)} stable tokens stranded behind break`:'no stable suffix stranded'} · ${compileText} · ${Array.isArray(frame.failedOutlets) && frame.failedOutlets.length ? `failed outlets ${frame.failedOutlets.join(', ')}` : 'all reported outlets settled'} · ${Array.isArray(frame.publicationRejections) ? frame.publicationRejections.length : 0} rejected publications` : 'A normal foreground generation will populate this after Nexus seals its context frame.';
    ...
    const lorePotentialGain=Number(loreCacheShadow?.potentialGainTokens)||0,loreRealizedGain=Number(loreCacheShadow?.realizedGainTokens)||0,loreMode=String(loreCacheShadow?.presentationMode||'shadow'),loreStrategy=String(loreCacheShadow?.presentationStrategy||'canonical');
    const loreCacheText=loreCacheShadow
        ?(loreMode==='active'
            ?(loreCacheShadow.hasPrior===true?`lore-order active ${loreStrategy} · +${formatTokenCount(loreRealizedGain)} realized prefix tokens vs canonical`:`lore-order active ${loreStrategy} · awaiting prior sample`)
            :(lorePotentialGain?`lore-order shadow +${formatTokenCount(lorePotentialGain)} potential prefix tokens`:'lore-order shadow no gain yet'))
        :'lore-order telemetry pending';
    const epochName=mainContext?.name||null,epoch=mainContext?.data||null;
    const promptAdapter=prompt?.nexusFrame?.promptLoaderAdapter||frame?.promptLoaderAdapter||null;
    const adapterVerification=telemetry?.latest?.promptLoader?.adapterVerification?.data||null;
    const promptAdapterBasis=promptAdapter?.evidence?`${promptAdapter.evidence.structure||'conservative-default'}; ${promptAdapter.evidence.cache||'conservative-prefix'}`:'basis n/a';
    const promptAdapterVerificationText=adapterVerification?(adapterVerification.matched===true?'request verified':`REQUEST MISMATCH sealed ${adapterVerification.sealed?.family||'?'} → actual ${adapterVerification.actual?.family||'?'}`):'request verification pending';
    const promptAdapterText=promptAdapter?`${promptAdapter.family||'Generic'} / ${promptAdapter.wrapperStyle||promptAdapter.layout||'default'} / ${promptAdapter.cachePolicy||'cache-policy n/a'} / ${promptAdapterBasis} / ${promptAdapterVerificationText}`:'adapter not recorded';
    const promptDetail = prompt ? `${prompt.messageCount ?? 0} messages · ${Number(prompt.nexusFrameSharePct || 0).toFixed(1)}% Nexus frame · ${prompt.generationId || 'unidentified generation'} · ${promptAdapterText} · ${prompt.stability?.identicalToPrevious===true?'identical to prior real prompt':(prompt.stability?.stablePrefixRatioPct!=null?`${prompt.stability.stablePrefixRatioPct}% / ${formatTokenCount(prefixTokens)} stable prefix${breakScope?` · break ${breakScope}`:''}`:'first real prompt sample')} · ${loreCacheText}${epochName?` · ${epochName.replace('cache-epoch-','epoch ')} ${epoch?.rawAssistantTurns??'?'} / ${epoch?.maxRawAssistantTurns??'?'} raw turns`:''}` : 'Dry-run observations do not replace this retained real-request snapshot.';
