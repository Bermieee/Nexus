...
function profilePlans(sourceTokens,profiles=['lean','balanced','heavy']){return profiles.map(id=>{const cfg=PROFILES[id]||PROFILES.balanced;return {id,label:cfg.label,detail:cfg.detail,targetTokens:profileCap(id,sourceTokens)};});}
...
function uidFinalPrompt(identity,plan,contributions,detail,includeKeywords){const refs=uidContributionRefs(contributions);return `NEXUS UID SUMMARIZER — FINAL DRAFT
